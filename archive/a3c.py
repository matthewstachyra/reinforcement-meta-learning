...
import numpy as np
from utils import convert_to_tensor_as_needed, \
...
class A3C(nn.Module):
    ...
    def forward(self, 
                state: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # relu6 is a modified relu where the output
        # is limited to 6
        a1_result = self.a1(state)
        print(f'a1 result {type(a1_result)}')
        a1 = F.relu6(a1_result)
    ...
    def loss_func(self, 
        ...
        # Total loss for the network is the average of the 'Critic'
        ...
        while self.g_ep.value < MAX_EP:
            ...
            for t in range(MAX_EP_STEP):
                ...
                a = self.lnet.choose_action(
                    convert_to_tensor_as_needed(s[0]))

                # We clip the action beceause Gaussian distribution
                # isn't bounded. This way we keep it in a valid range.
                # The new gym API returns 5 values rather than 4. You 
                # are able to use the temporary wrapper to get the old
                # functionality, but we don't do that there. The extra 
                # value is due to the 'done' boolean being split into
                # two values - 'terminated' and 'truncated'. This captures
                # extra nuance which is when the episode ends due to a time
                # limit or a reason that is not defined as part of the task
                # MDP.
                s_, r, done, _, _ = self.env.step(a.clip(-2, 2))
                print(f"s_: {s_}, {type(s_)}")
